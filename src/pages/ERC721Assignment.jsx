...
const ERC721Assignment = () => {
  ...
  useEffect(() => {
    ...
    if( contract && currentAccount ){
      let myBalance = getContractBalanceOf( currentAccount );
      myBalance.then( (res) => {
  ...
  async function onContractMint( account, price ){
    let pr = ethers.utils.parseEther( price );
    return await contract.mint( { from : account, value : pr.toString() } );
  }
  ...
  const onMint = () => {
    ...
    if( contract && currentAccount ){
      ...
      if( mintPrice )
      {
        onContractMint( currentAccount, mintPrice );
      }
  ...
  useEffect(() => {
    ...
    let interval = window.setInterval( () => {
      if( contract ){
        let totalSupply = getContractTotalSupply();
        totalSupply.then( (res) => {
          setTotalSupply( res.toNumber() );
        } );
      }
      if( currentAccount ){
        let balanceOf = getContractBalanceOf( currentAccount );
        balanceOf.then( (res) => {
          setAccountBalance( res.toNumber() );
        } );
      }
          ...
          <ul>
             ...
             */
              [...new Array(totalSupply)].map( ( item, index ) => {
                return <OwnerListItem key={index} tokenId={ index } contract={ contract }/>
              } )
